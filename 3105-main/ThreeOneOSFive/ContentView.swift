...
struct GameDemoView: View {
    ...
    private let features: [DemoFeature] = [
        DemoFeature(name: "Proxy Aim Body", description: "File đang được bảo trì", implemented: false, bundledFileName: nil),
        DemoFeature(name: "Proxy Aim Neck V2", description: "Đã đính kèm file demo Aim Chest.3105", implemented: true, bundledFileName: "Aim Chest.3105"),
            ...
            ZStack {
                ...
                ScrollView(showsIndicators: false) {
                    VStack(spacing: 18) {
                        ...
                        if selectedTab == 0 { featureList }
                        else if selectedTab == 1 { DemoMessageCard(title: "Định Vị", text: "Khu vực giao diện demo. Chưa thực hiện can thiệp vào game.", icon: "location.fill") }
                        else { DemoMessageCard(title: "Mod NV", text: "Khu vực giao diện demo. Chưa thực hiện can thiệp vào game.", icon: "person.2.fill") }
                    }
                    .padding(16)
                }
            }
    ...
    private var featureList: some View {
        VStack(alignment: .leading, spacing: 10) {
            Text("CHỨC NĂNG PROXY").font(.caption.weight(.bold)).foregroundStyle(.secondary).padding(.horizontal, 4)
            ForEach(features.indices, id: \.self) { index in
                HStack(spacing: 12) {
                    Image(systemName: "wrench.and.screwdriver.fill").foregroundStyle(AppTheme.rowColor(index + 2)).frame(width: 38, height: 38).background(AppTheme.rowColor(index + 2).opacity(0.16), in: RoundedRectangle(cornerRadius: 11))
                    VStack(alignment: .leading, spacing: 2) {
                        Text(features[index].name).font(.headline)
                        Text(features[index].description).font(.caption).foregroundStyle(.secondary)
                    }
                    Spacer()
                    if busyIndex == index {
                        ...
                        Toggle("", isOn: Binding(
                            ...
                            set: { value in handleFeature(index: index, value: value) }
                        ))
                        .labelsHidden().tint(.cyan)
                    }
                }
                .padding(12).background(Color.white.opacity(0.035), in: RoundedRectangle(cornerRadius: 14))
            }
            Text("Các mục chưa có module hợp lệ sẽ tự trở về Tắt và báo bảo trì.")
                .font(.caption).foregroundStyle(.secondary).padding(.top, 4)
        }
        .padding(14).proxyCard()
    }

    private func handleFeature(index: Int, value: Bool) {
        guard value else { enabled.remove(index); return }
        busyIndex = index
        Task { @MainActor in
            try? await Task.sleep(nanoseconds: 3_000_000_000)
            busyIndex = nil

            if features[index].implemented, let bundledFileName = features[index].bundledFileName {
                if copyBundledFeatureFile(named: bundledFileName) {
                    enabled.insert(index)
                    maintenanceText = "\(features[index].name) đã bật thành công. File demo đã được nạp vào bộ nhớ ứng dụng."
                } else {
                ...
                enabled.remove(index)
                maintenanceText = "\(features[index].name) đang được bảo trì. Nút đã tự tắt."
            }
